...
unsigned int nTargetSpacing = 1 * 160;
...
static const int64_t nTargetTimespan = 30 * 160;
...
const string strMessageMagic = "Rime Signed Message:\n";
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree,
                        bool* pfMissingInputs, bool fRejectRimeFee, bool ignoreFees)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
...

    {
        ...
        if (fRejectRimeFee && nFees > MIN_RELAY_TX_FEE * 10000)
            return error("AcceptableInputs: : rime fees %s, %d > %d",
                         hash.ToString(),
...
bool AcceptableInputs(CTxMemPool& pool, const CTransaction &txo, bool fLimitFree,
                         bool* pfMissingInputs, bool fRejectRimeFee, bool isDSTX)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
        *pfMissingInputs = false;
...

    {
        ...
        if (fRejectRimeFee && nFees > txMinFee * 10000)
            return error("AcceptableInputs: : rime fees %s, %d > %d",
                         hash.ToString(),
                         nFees, MIN_RELAY_TX_FEE * 10000);
...
bool CMerkleTx::AcceptToMemoryPool(bool fLimitFree, bool fRejectRimeFee, bool ignoreFees)
{
    return ::AcceptToMemoryPool(mempool, *this, fLimitFree, NULL, fRejectRimeFee, ignoreFees);
}
...
unsigned int GetNextTargetRequired_legacy(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    LogPrintf("Rime retargetted using: Terminal-Velocity difficulty curve \n");
    return Terminal_Velocity_RateX(pindexLast, fProofOfStake);
...
static unsigned int GetNextTargetRequired_new(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes RimeCoin fix for wrong retargeting difficulty by Mammix2
