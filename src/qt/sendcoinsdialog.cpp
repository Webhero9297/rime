    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Rime address (e.g. MLaZBUkdM15PjB7Zmbmwfwi31L3KeExgFG)"));
#endif
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CRimeAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::IXTransactionCreationFailed:
        msgParams.first = tr("FastTx doesn't support sending values that high yet. Transactions are currently limited to 500 RIME.");
        msgParams.second = CClientUIInterface::MSG_ERROR;
        break;
    ...
    case WalletModel::NarrationTooLong:
        ...
        break;
    case WalletModel::RimeFee:
        msgParams.first = tr("A fee %1 times higher than %2 per recipient is considered an rimely high fee.").arg(10000).arg(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), MIN_RELAY_TX_FEE));
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CRimeAddress addr = CRimeAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Rime address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
