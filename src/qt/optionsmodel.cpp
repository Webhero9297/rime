...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeRimeAmount"))
        settings.setValue("nAnonymizeRimeAmount", 1000);
    nAnonymizeRimeAmount = settings.value("nAnonymizeRimeAmount").toLongLong();
    if (settings.contains("nAnonsendRounds"))
        SoftSetArg("-anonsendrounds", settings.value("nAnonsendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeRimeAmount"))
        SoftSetArg("-anonymizeRimeamount", settings.value("nAnonymizeRimeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeRimeAmount:
            return QVariant(nAnonymizeRimeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case AnonsendRounds:
            ...
            break;
        case AnonymizeRimeAmount:
            nAnonymizeRimeAmount = value.toInt();
            settings.setValue("nAnonymizeRimeAmount", nAnonymizeRimeAmount);
            emit AnonymizeRimeAmountChanged(nAnonymizeRimeAmount);
            break;
        default:
            break;
        }
