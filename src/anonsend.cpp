    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Rime and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002 RIME per transaction on average.
            */
            if(r <= 10)
            {
...
bool CAnonsendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nLowestDenom = ANONSEND_COLLATERAL + anonSendDenominations[anonSendDenominations.size() - 1]*5;
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeRimeAmount*COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nAnonsendRounds))
    {
        nValueIn = 0;
        vCoins.clear();
...
void CAnonsendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100 RIME +1 ( bit on if present )
    // bit 1 - 10 RIME +1
    // bit 2 - 1 RIME +1
    // bit 3 - .1 RIME +1
    // bit 3 - non-denom
