...
//
// Bootup the masternode, look for a 5,000 RIME input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in Rime.conf */
...
{
    CRimeAddress address(collateralAddress);
